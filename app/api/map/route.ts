...
async function fetchTmdbShow(showId: number): Promise<TmdbShow | null> {
  ...
  try {
    const response = await fetch(
      `https://api.tmdb.org/3/tv/${showId}?api_key=${process.env.TMDB_API_KEY}&language=en-US`,
    );
...
async function fetchTmdbSeason(
  ...
  try {
    const response = await fetch(
      `https://api.tmdb.org/3/tv/${showId}/season/${seasonNumber}?api_key=${process.env.TMDB_API_KEY}&language=en-US`,
    );
