...
export async function fetchTMDBData<T = MediaItem>(
  ...
  // Only append videos and images for detail endpoints (e.g., /movie/{id} or /tv/{id})
  // List endpoints and discover endpoints don't support append_to_response for individual items
  ...
  // We'll handle 'page' separately to avoid duplication
  const finalParams = { ...filteredParams };
  if (page > 1 || !finalParams.page) {
    finalParams.page = page.toString();
  }

  for (const [key, value] of Object.entries(finalParams)) {
    if (typeof key === "string" && typeof value === "string") {
      // Avoid adding sort_by to non-discover/non-search endpoints that might not support it
      if (
        key === "sort_by" &&
      ...
      // Ensure we don't append duplicate page if it was already added or if we are about to
  ...
  // If page wasn't added yet, add it
