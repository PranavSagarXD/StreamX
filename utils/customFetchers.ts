...
async function fetchByDirector(
  ...
  try {
    const response = await fetch(
      `https://api.tmdb.org/3/person/${personId}/movie_credits?api_key=${process.env.TMDB_API_KEY}&language=en-US`,
    );
...
async function fetchByStudio(
  ...
  try {
    const response = await fetch(
      `https://api.tmdb.org/3/discover/movie?api_key=${process.env.TMDB_API_KEY}&with_companies=${companyId}&sort_by=popularity.desc&page=${page}&language=en-US&include_adult=false&vote_count.gte=20&vote_average.gte=5.5`,
    );
...
async function fetchByCollection(
  ...
  try {
    const response = await fetch(
      `https://api.tmdb.org/3/collection/${collectionId}?api_key=${process.env.TMDB_API_KEY}&language=en-US`,
    );
...
async function fetchDiverseTV(
  ...
  for (const genre of genres) {
    try {
      const response = await fetch(
        `https://api.tmdb.org/3/discover/tv?api_key=${process.env.TMDB_API_KEY}&with_genres=${genre.id}&with_origin_country=US&language=en-US&include_adult=false&sort_by=popularity.desc&page=${page}&vote_count.gte=80&vote_average.gte=6.5&without_genres=99,10763,10767&first_air_date.gte=2010-01-01`,
      );
...
async function fetchSitcoms(
  ...
  const response = await fetch(
    `https://api.tmdb.org/3/discover/tv?api_key=${process.env.TMDB_API_KEY}&with_genres=35&without_genres=16,18,80,10759,10765,9648&with_origin_country=US&vote_average.gte=7.0&vote_count.gte=50&first_air_date.gte=1970-01-01&first_air_date.lte=2020-12-31&sort_by=popularity.desc&page=${page}`,
  );
...
async function fetchKidsNetworks(
  ...
  for (const network of networks) {
    try {
      const response = await fetch(
        `https://api.tmdb.org/3/discover/tv?api_key=${process.env.TMDB_API_KEY}&with_networks=${network.id}&language=en-US&include_adult=false&sort_by=popularity.desc&page=${page}&vote_count.gte=20&vote_average.gte=5.5`,
      );
...
async function fetchNetworkHits(
  ...
  for (const network of networks) {
    try {
      const response = await fetch(
        `https://api.tmdb.org/3/discover/tv?api_key=${process.env.TMDB_API_KEY}&with_networks=${network.id}&language=en-US&include_adult=false&sort_by=popularity.desc&page=${page}&vote_count.gte=50&vote_average.gte=6.0`,
      );
...
async function fetchUpcomingMovies(
  ...
  try {
    ...
    const response = await fetch(
      `https://api.tmdb.org/3/discover/movie?api_key=${process.env.TMDB_API_KEY}&primary_release_date.gte=${todayString}&primary_release_date.lte=${twelveMonthsString}&sort_by=popularity.desc&page=${page}&language=en-US&region=US&include_adult=false`,
    );
...
async function validateMovieReleaseDate(
  ...
  try {
    const response = await fetch(
      `https://api.tmdb.org/3/movie/${movieId}/release_dates?api_key=${process.env.TMDB_API_KEY}`,
    );
