...
async function fetchDetails(id: string) {
  try {
    const response = await fetch(
      `https://api.tmdb.org/3/movie/${id}?api_key=${process.env.TMDB_API_KEY}&language=en-US&append_to_response=videos,images,credits,recommendations,similar,keywords,reviews`,
    );
    if (!response.ok) {
