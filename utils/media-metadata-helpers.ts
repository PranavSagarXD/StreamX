...
export async function generateMediaMetadata({
  ...
  if (!media) {
    return {
      title:
        notFoundTitle ||
        `${mediaType === "movie" ? "Movie" : "TV Show"} Not Found | StreamX`,
      description:
        notFoundDescription ||
  ...
  const description = media.overview || `Watch ${title} on StreamX`;
  ...
  return {
    ...
    ...(mediaType === "movie" && {
      alternates: {
        canonical: `https://StreamX.com/movies/${mediaId}`,
        languages: {
          "en-US": `https://StreamX.com/movies/${mediaId}`,
        },
      },
    }),
    description,
